...
def conv_net(x_dict, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope("ConvNet", reuse=reuse):
        ...
        # Convolution Layer with 32 filters(channels) and the kernel size is 5
        ...
        # Convolution Layer with 64 filters(channels) and a kernel size of 3
...
def model_fn(features, labels, mode, params):
    ...
    # 如果没有dropout那么就可以不区分，类似之前simple_nn_high中的做法，直接调用一个搞定。
    logits_train = conv_net(features, num_classes, params['dropout'], reuse=False,
                            is_training=True)
    logits_test = conv_net(features, num_classes, params['dropout'], reuse=True,
                           is_training=False)
...
# Build the Estimator
model = tf.estimator.Estimator(model_fn=model_fn, model_dir="cnn_high", params=model_params)
